...
myDropzone.on('addedfile', function(file){
    ...
    if (file.size > 2 * 1024 * 1024) {
        ...
        Swal.fire({ 
            title: "Mesa de Partes", 
            text: 'El archivo "'+ file.name +'" excede el tamaño de 2 MB.', 
            icon: "error", 
            confirmButtonColor: "#5156be" 
          });
...
function ver(doc_id){
    ...
    $.post("../../controller/documento.php?op=mostrar", {doc_id : doc_id}, function(data){
        ...
        $("#lbltramite").html("Nº Trámite: " + data.nrotramite + " | Usuario: " + data.usu_nomape + " | Correo: " + data.usu_correo + " | Adjuntos: " + data.cant + " | Estado: " + resultado);
...
function guardar(e){
    ...
    if (arrDocument.length === 0) {
        ...
        Swal.fire({
            title: "No hay archivos adjuntos, está seguro de responder el trámite?",
            icon: "info",
            showDenyButton: true,
...
function enviartramite(){
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            Swal.fire({ 
                title: "Mesa de Partes", 
                html: "Su trámite ha sido respondido con éxito e informado al usuario, con el Nº: <br><strong>"+ data +"</strong>", 
                icon: "success", 
                confirmButtonColor: "#5156be" 
              });
